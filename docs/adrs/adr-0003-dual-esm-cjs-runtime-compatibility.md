...
`@plasius/profile` publishes dual ESM and CJS entry points. This package currently emits CJS output under `dist-cjs/*.js` while the repository root uses `type: module`. Without an explicit CommonJS boundary for `dist-cjs`, Node can interpret those files as ESM and fail at runtime for `require(...)` consumers.
...
- Node CommonJS consumers can reliably load `@plasius/profile` via `require(...)`.
